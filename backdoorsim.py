...
def investigation_shell(): # Creates another new function to begin simulating the shell for investigating the threat
  ...
  while True: # Starts the command loop
    ...
    if cmd == "help": # Show the available commands to the user when they type 'help'
      ...
      print(f"HINT: {get_hint(scenario_stage)}") # Gives a hint message to the user based on the current step in the investigation
      print("Please type your next command.")
      continue
      ...
      print_summary(user_choice) # If the user types exit it will not just exit but print the summary of the investigation done
...
def get_hint(stage): # Creates a new function that take one parameter 'stage'
  ...
  return hints.get(stage, "No hints available for this step.") # Returns the hint for the given stage if it exists if not it returns 'No hints available for this step'

def print_summary(user_choice): # Creates a new function for showing the summary at the end for the user to look over
  print("\n--- Investigation Summary ---")
  stages_done = set(stage for stage, cmd in user_choice) # Creates a set of unique investigation stages the user has done by taking away the stage from each (stage, cmd) pair in the user_choice list
